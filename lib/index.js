    ...
    function SimpleCRUD(requiredProps, strictMode, defaultData) {
        ...
        this.update = function (objectToUpdate, option, updateProp) {
            ...
            if (option !== set && option !== remove)
                throw new Error("There is only 'set' and 'remove' options");
            //@ts-ignore
            ...
            if (_this.strictMode) {
                ...
                if (updateObjects_1.length === 0 || !updateObjects_1)
                    return;
            ...
            if (_this.strictMode) {
                ...
                _this.containerArray.forEach(function (object, index) {
                    ...
                    keys_2.forEach(function (key) {
                        if (key === 'id') {
                            //@ts-ignore
                            if (object.id.toString() === deleteProp.id.toString())
                                counter++;
                        }
                        else {
                            //@ts-ignore
        ...
        if (defaultData) {
            defaultData.forEach(function (data) {
                for (var _i = 0, _a = _this.requiredProps; _i < _a.length; _i++) {
                    ...
                    //@ts-ignore
                    if (!data[prop])
                        throw new Error("Data you provided does not have the required properties");
                }
            });
